...
const request = async (req, res) => {
  ...
  const doc = await User.findOne({email: email}).then((doc) => {
    if (doc) {
      const uniquekey = uuid();
      const someDate = new Date();
      const numberOfDaysToAdd = 1;
      const expirydate = someDate.setDate(
      ...
      const output = `
              ...
              <h3>Details</h3>
              <ul>
                <a href="${process.env.CLIENT_URL}/password/reset/${uniquekey}">Click here!</a>
              </ul>
            `;
      ...
      return res
        .status(400)
        .json({success: false, message: "I can't find you."});
    }
  });
};
        ...
        .then((user) => {
          ...
          user
            ...
            .catch((err) => {
              ...
              res
                .status(400)
                .json({success: false, message: 'Failed miserably....'});
            });
        })
        .catch((err) => res.status(400).json('Error: ' + err));
    } else {
